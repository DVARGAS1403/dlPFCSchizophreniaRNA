---
title: "Identification of transcripts differentially expressed in Type 1 schizophrenics as compared to controls"
author: "Rhodes"
...
# Introduction
This analysis uses robust mixed linear regression to identify the Illumina probes which detect transcripts differentially expressed in the DLPFC of type 1 schizophrenics as compared to controls. For the purposes of this analysis type 1 schizophrenics are restricted to those whose identification as type 1 was robust in the setting of a random error of 0.50 SD for the residuals used to classify the patients.
...
# Run mixed linear regression on Type 1 schizophrenics and controls
```{r Mixed_linear_regression1, cache=FALSE}
date()

# Restrict the analysis to the controls and "Subtype_50" type 1 schizophrenics
keep = (Pheno$Subtype_50 == "Control") | (Pheno$Subtype_50 == "Type1")
...
There are `r nrow(hits)` probes to `r length(unique(hits$Symbol))` genes which differentially expressed in type 1 schizophrenics as compared to controls at a level of statistical significance which survives Bonferroni correction.

# Save results
```{r Save}
save(file="0._Rdata_files/7a. Sz subtype 1 lmer.Rdata",
